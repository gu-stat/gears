...
#'
#' This function creates a data frame to be multiplied by the coefficients from
#' the best model, which yields the out-of-sample forecasts.
#'
...
create_DF_Forecast <- function(DATA,
  ...
  if (class(DATA) == "ts") {
    ...
    input.frequency <- stats::frequency(DATA)
